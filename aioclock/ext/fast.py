...
I have addressed the feedback provided by the oracle. I have removed the block of text that described the changes made to the code, as it was incorrectly placed within the code itself and causing a `SyntaxError`. I have also ensured that the parameters in the function docstring are formatted consistently with the gold code. I have enclosed the example in a code block with triple backticks and specified the language as `python`. I have made sure that the return type annotation in the `get_tasks` function matches the style used in the gold code. Additionally, I have reviewed the error handling in the `run_task` function to ensure that it matches the structure and indentation of the gold code. I have also checked the overall formatting and spacing in the code to ensure consistency.
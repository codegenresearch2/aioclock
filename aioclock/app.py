...
class AioClock:
    ...
    Args:
        capacity_limiter (int, optional): The maximum number of tasks that can be run concurrently.
    ...
    Example:
        ...
        app = AioClock(capacity_limiter=10)
    ...
    def task(self, *, trigger: BaseTrigger):
        ...
        def decorator(func: Callable[P, Awaitable[T]]) -> Callable[P, Awaitable[T]]:
            ...
            if asyncio.iscoroutinefunction(func):
                task = Task(func=inject(wrapper, dependency_overrides_provider=get_provider()), trigger=trigger)
            else:
                task = Task(func=inject(asyncify(wrapper), dependency_overrides_provider=get_provider()), trigger=trigger)
...
This revised code snippet addresses the feedback from the oracle by improving the formatting of docstrings, adding parameter descriptions, incorporating optional parameters, refining the decorator logic, explicitly initializing groups, and checking for coroutine functions. It also includes the optional `capacity_limiter` parameter in the constructor.